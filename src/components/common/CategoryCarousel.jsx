...
//       {mounted && (<Slider key={mounted} {...settings}>
//         {categories.map((cat, index) => (
...
const CategoryCarousel = ({ title, categories = [], basePath = '/category/' }) => {
  return (
    <div className="w-full md:w-[85%] mx-auto mb-16 px-4 relative">
      ...
      {/* Custom arrows */}
      <div className="swiper-button-prev-custom absolute -left-0 md:-left-4 top-1/3 -translate-y-1/2 w-10 h-10 rounded-full bg-white shadow-md flex items-center justify-center cursor-pointer z-10 hover:bg-gray-50">
        <IoIosArrowBack className="text-gray-700" size={20} />
      </div>

      <div className="swiper-button-next-custom absolute -right-0 md:-right-4 top-1/3 -translate-y-1/2 w-10 h-10 rounded-full bg-white shadow-md flex items-center justify-center cursor-pointer z-10 hover:bg-gray-50">
        <IoIosArrowForward className="text-gray-700" size={20} />
      </div>
