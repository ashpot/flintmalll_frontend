...
const Step_EnterNewPassword = ({ onPasswordSet, isLoading }) => {
  const [pass, setPass] = useState('');
  const [confirm, setConfirm] = useState('');

  const handleSubmit = (e) => {
    e.preventDefault();
    if (pass === confirm) onPasswordSet(pass);
  ...
  return (
    <form onSubmit={handleSubmit} className="text-left space-y-4">
      ...
      <button 
        type="submit" 
        disabled={isLoading}
        className="w-full py-4 bg-secondary text-white rounded-xl font-bold"
      >
